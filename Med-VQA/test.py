...
def get_result(model, dataloader, device, args):
    ...
    with torch.no_grad():
        for v, q, a, ans_type, q_types, p_type in iter(dataloader):
            ...
            # print(get_question(q, dataloader))
            # print(get_answer(preds, dataloader))
            # print(get_answer(a, dataloader))
            # plt.title('Question: ' + get_question(q, dataloader) + '\nPrediction: ' + get_answer(preds, dataloader) + '\nAnswer: ' +  get_answer(a, dataloader))
            # plt.savefig('tmp/test_result%d.png' %count, dpi=100)
            # plt.show()
...
def prediction(image_path, question, model, device, args, dataloader):
    with torch.no_grad():
        ...
        dictionary = dataloader.dataset.dictionary
        qst2idc = np.array([dictionary.word2idx('_')] * 30)
        qst2idc[:len(qst)] = [dictionary.word2idx(w) for w in qst]
        qst2idc = qst2idc.reshape(1,30)

        q = torch.from_numpy(qst2idc).to(device)
        v = image.to(device)
        features = model(v, q)
        preds = model.classifier(features)
        plt.title('Question: ' + question + '\nPrediction: ' + get_answer(preds, dataloader))
        plt.savefig('pred_result%d.png' %count, dpi=100)
        plt.show()
...
if __name__ == '__main__':
    ...
    image = './data_RAD/images/synpic9872.jpg'
    question = 'What is this'
    prediction(image, question, model, args.device, args, eval_loader)
